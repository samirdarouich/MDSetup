...
from mdsetup.tools.general import add_nan_if_no_brackets, flatten_list, generate_series
...
def read_gromacs_xvg(file_path: str, time_fraction: float = 0.0):
    ...
    Description:
    This function reads data from a Gromacs XVG file specified by 'file_path'. It
    extracts the data columns and their corresponding properties from the file. The data
    is then filtered based on the 'time_fraction' parameter, selecting only the data
    points that are within the specified time_fraction of the maximum time value.
    The selected data is returned as a pandas DataFrame, where each column
    represents a property and each row represents a data point.
    ...
    # In case special properties are delivered, there is just one regular property,
    # which is given for every special property.
    ...
    # As special properties only have a mean and a std. Generate a series that exactly
    # has the mean and the standard deviation of this value.
