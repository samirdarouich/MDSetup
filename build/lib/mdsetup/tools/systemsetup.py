...
from mdsetup.forcefield.reader import extract_number_dict_from_mol_files
        ...
        if software == "gromacs":
            ...
            if "p" in ensemble_settings.keys():
                ...
                ensemble_settings["p"].update(
                    {
                        "ref_p": "   ".join([str(pressure)] * no_coupl),
                        "compressibility": "   ".join(
                            [str(kwargs["compressibility"])] * no_coupl
                        ),
                    }
                )
        ...
        for j, step in enumerate(ensemble_names):
            ...
            if ensembles[j - 1] == "em" or ensembles[j] == "em" or not cpt_relative[j]:
                job_file_settings["ensembles"][step]["grompp"] = (
                    f"grompp -f {mdp_relative[j]} -c {cord_relative[j]} -r {cord_relative[j]} -p {topo_relative[j]} -o {out_relative[j]}"
                )
            else:
                job_file_settings["ensembles"][step]["grompp"] = (
                    f"grompp -f {mdp_relative[j]} -c {cord_relative[j]} -r {cord_relative[j]} -p {topo_relative[j]} -t {cpt_relative[j]} -o {out_relative[j]}"
                )
