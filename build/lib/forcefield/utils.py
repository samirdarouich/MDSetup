...
def check_pair_style(
    style, ffiatom: Dict[str, str | float], ffjatom: Dict[str, str | float]
):
    if ffiatom[style] != ffjatom[style]:
        raise KeyError(
            f"Atom '{ffiatom['name']}' and atom '{ffjatom['name']}' has different {style}:\n  {ffiatom[style]} vs {ffjatom[style]}"
        )


def get_mixed_parameters(
    ffiatom: Dict[str, float | str],
    ffjatom: Dict[str, float | str],
    ...
    if mixing_rule == "arithmetic":
        ...
        raise KeyError(
            f"Specified mixing rule is not implemented: '{mixing_rule}'. Valid options are: 'arithmetic', 'geometric', and 'sixthpower'"
        )
...
def get_pair_style(
    ...
    vdw_pair_styles: List[str],
    coul_pair_styles: List[str],
    pair_style_kwargs: Dict[str, str],
):
    """
    ...
    for vdw_pair_style in set(vdw_pair_styles):
        sub_string = f"{vdw_pair_style} " + " ".join(
            [str(local_attributes[arg]) for arg in pair_style_kwargs[vdw_pair_style]]
        )
        combined_pair_style.append(sub_string)

    for coul_pair_style in set(coul_pair_styles):
        sub_string = f"{coul_pair_style} " + " ".join(
            [str(local_attributes[arg]) for arg in pair_style_kwargs[coul_pair_style]]
        )
        combined_pair_style.append(sub_string)
