...
def convert_harmonic_bond( source: str, destination: str, bond: Dict[str,str|float|int] ):
    ...
    if source == "GROMACS" and destination == "LAMMPS":
        # [r, K] to [K, r]
        new_bond["p"] = [ bond["p"][1] / 2 / 4.184 / 100, bond["p"][0] * 10 ]
        new_bond["style"] = "harmonic"

    elif source == "LAMMPS" and destination == "GROMACS":
        # [K, r] to [r, K]
        new_bond["p"] = [ bond["p"][1] / 10, bond["p"][0] * 2* 4.184 * 100 ]
        new_bond["style"] = 1
    else:
...
def convert_harmonic_angle( source: str, destination: str, angle: Dict[str,str|float|int] ):
    ...
    if source == "GROMACS" and destination == "LAMMPS":
        ...
        new_angle["p"] = [ angle["p"][1] / 2 / 4.184, angle["p"][0] ]
        new_angle["style"] = "harmonic"

    elif source == "LAMMPS" and destination == "GROMACS":
        # [K, theta0] to [theta0, K]
        new_angle["p"] = [ angle["p"][1], angle["p"][0] * 2* 4.184 ]
        new_angle["style"] = 1
    else:
        source_destination_error( source , destination )
...
def convert_harmonic_dihedral( source: str, destination: str, dihedral: Dict[str,str|float|int] ):
    ...
    if source == "GROMACS" and destination == "LAMMPS":
        ...
        new_dihedral["p"] = [ dihedral["p"][1] * 4184, -1, dihedral["p"][2] ]
        ...
        new_dihedral["p"] = [ 0.0 if dihedral["p"][1]==-1 else 180.0, dihedral["p"][0] / 4184, dihedral["p"][2] ]
...
def convert_opls_dihedral( source: str, destination: str, dihedral: Dict[str,str|float|int] ):
    ...
    if source == "GROMACS" and destination == "LAMMPS":
        ...
        new_dihedral["p"] = [6, *[ p/4.184*(-1)**i for i,p in enumerate(dihedral["p"]) ] ]
        ...
        opls = np.array(dihedral["p"][1:]).reshape(-1,1)
        new_dihedral["p"] = (A*opls).flatten().tolist()[0]
        new_dihedral["style"] = 1
    else:
        source_destination_error( source ,destination )
...
def convert_atom( source: str, destination: str, atom: Dict[str,str|float|int] ):
    ...
    if source == "GROMACS" and destination == "LAMMPS":
        new_atom["epsilon"] = round( atom["epsilon"] / 4.184, 4 ) # convert from kJ to kcal
        new_atom["sigma"] = round( atom["sigma"] * 10, 4 ) # convert from nm to A 
        new_atom["vdw_style"] = "lj/cut"
        new_atom["coul_style"] = "coul/long" if atom["charge"] != 0 else ""
    elif source == "LAMMPS" and destination == "GROMACS":
        new_atom["epsilon"] = round( atom["epsilon"] * 4.184, 4 ) # convert from kcal to kJ
        new_atom["sigma"] = round( atom["sigma"] / 10, 4 ) # convert from A to nm 
...
def convert_angle( source: str, destination: str, angle: Dict[str,str|float|int] ):
    new_angle = ANGLE_MAP[bond["style"]]( source, destination, angle )

    return new_angle

def convert_dihedral( source: str, destination: str, dihedral: Dict[str,str|float|int] ):
    new_dihedral = DIHEDRAL_MAP[bond["style"]]( source, destination, dihedral )
