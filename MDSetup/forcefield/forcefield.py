...
class forcefield():
    ...
    def __init__(self, smiles: List[str], force_field_paths: List[str] ):
        ...
        self.dihedrals = unique_by_key( flatten_list( molecule.map_molecule( molecule.unique_torsion_keys, self.ff["dihedrals"] ) for molecule in self.mol_list ), "list" ) 
        
        if not all( [ all(self.nonbonded), all(self.bonds), all(self.angles), all(self.dihedrals) ] ):
            txt = "nonbonded" if not all(self.nonbonded) else "bonds" if not all(self.bonds) else "angles" if not all(self.angles) else "dihedrals"
        ...
        labelmap = {}
        labelmap["atoms"] = { i+1: atom_ff["name"] for i,atom_ff in enumerate( self.nonbonded ) }
        labelmap["bonds"] = { i+1: "_".join( sort_force_fields( bond_ff["list"] ) ) for i,bond_ff in enumerate( self.bonds ) }
        labelmap["angles"] = { i+1: "_".join( sort_force_fields( angle_ff["list"] ) ) for i,angle_ff in enumerate( self.angles ) }
        labelmap["dihedrals"] = { i+1: "_".join( sort_force_fields( dihedral_ff["list"] ) ) for i,dihedral_ff in enumerate( self.dihedrals ) }
        
    def write_molecule_files( self, molecule_template: str, molecule_path: str,
                              residues: List[str], **kwargs ):
        """
        ...
        if not os.path.exists(molecule_template):
            raise FileExistsError(f"Molecule template does not exists:\n   {molecule_template}")
        ...
        file_suffix = "itp" if self.software.lower() == "gromacs" else "mol" if self.software.lower() == "LAMMPS" else ""
        ...
        for m,mol in enumerate(self.mol_list):
            ...
            if self.software.lower() == "gromacs":
                ...
                self.gro_files.append( write_gro_file( mol, destination = molecule_path, **kwargs ) )
            ...
            renderdict.update( atoms_molecule( molecule_ff = molecule_ff, software = self.software, **kwargs ) )

            # Get bond information
            bond_list = mol.bond_list + 1 
            bond_names = mol.bond_names

            renderdict.update( bonds_molecule( bond_list = bond_list, bond_names = bond_names, 
                                               software = self.software, **kwargs )
                             )
            ...
            renderdict.update( angles_molecule( angle_list = angle_list, angle_names = angle_names,
                                                software = self.software, **kwargs )
                             )
            ...
            renderdict.update( dihedrals_molecule( dihedral_list = dihedral_list, dihedral_names = dihedral_names, 
                                                   software = self.software, **kwargs )
                             )

            # Get number of atoms, bonds, angles and dihedrals
            renderdict.update( { "atom_no": len(molecule_ff), "bond_no": len(bond_list), 
                                 "angle_no": len(angle_list), "dihedral_no": len(dihedral_list) } 
                                )
    ...
    def write_topology_file( self, topology_template: str, topology_path: str, system_name: str, **kwargs ):
        """
        ...
        if not os.path.exists(topology_template):
            raise FileExistsError(f"Topology template does not exists:\n   {topology_template}")
        ...
        file_suffix = "top" if self.software.lower() == "gromacs" else "params" if self.software.lower() == "LAMMPS" else ""
        ...
        if self.software.lower() == "lammps":
            ...
            KwargsError( ["potential_kwargs", "rcut", "do_mixing", "mixing_rule"], kwargs.keys() )

            # Provide label map
            kwargs["labelmap"] = self.labelmap

            # Get styles
            style_dict, hybrid_dict = style_topology( system_ff = self.nonbonded, bonds_ff = self.bonds, 
                                                      angles_ff = self.angles, dihedrals_ff = self.dihedrals, 
                                                      **kwargs 
                                                    )
            ...
            # Check necessary kwargs
            KwargsError( ["mixing_rule", "fudgeLJ", "fudgeQQ", "residue_dict"], kwargs.keys() )

        renderdict.update( atoms_topology( system_ff = self.nonbonded, software = self.software, **kwargs ) )
        renderdict.update( bonds_topology( bonds_ff = self.bonds, software = self.software, **kwargs ) )
        renderdict.update( angles_topology( angles_ff = self.angles, software = self.software, **kwargs ) )
        renderdict.update( dihedrals_topology( dihedrals_ff = self.dihedrals, software = self.software, **kwargs ) )
